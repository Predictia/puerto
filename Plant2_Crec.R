...
setkey(pl1$A2,t)
A2t<-pl1$A2[.(tt)];setkey(A2t,IDRebano)
...
#Calcular VPhift
setkey(pl1$A0,IDRebano);setkey(A2tD1,IDRebano)
...
FA2<-pl1$A6[FA1][,.(PMtot=sum(n*PV^0.67)),keyby=.(Especie,IDRebano)]
setkey(FA2,IDRebano);setkey(A0A2tD1D2B2,IDRebano)
FA3<-FA2[A0A2tD1D2B2][,.(IDRebano,Especie,PMtot,UP,IDMancha,com,com2,t)]
...
VPhift<-T4[FA4][,list(IDRebano,Especie,PMtot,UP,IDMancha,com,com2,IDHL,IDHL2,sup,
      ...
      VP:=NDig*ch*Fantin*Fmos*Tri(prbiovta,selectmin,selectmax,0,1)*Ipend][,
      NhecesVhif:=(1-DigV)*Npla][,
      NhecesMhif:=(1-DigM)*Npla*prtvm][,
      NorinVhif:=DigV*(1-qm)*Npla][,
      NorinMhif:=DigM*(1-qm)*Npla*prtvm][,
      vph:=sum(VP),by=.(IDRebano,Especie,t)][,
      diashift0:=ifelse(vph==0,0,VP/vph)][,
      prcom1:=sum(diashift0),by=.(IDRebano,com2,Fantin)][,
      corhft:=ifelse(prcom1>Fantin,Fantin/prcom1,1)][]

setkey(VPhift,IDRebano)
DIASht0<-unique(VPhift[,.(IDRebano)]);setkey(DIASht0,IDRebano)
setkey(VPhift,IDRebano,com2,Fantin,prcom1,corhft)
DIAShft0<-unique(VPhift[,.(IDRebano,com2,Fantin,prcom1,corhft)])
setkey(DIAShft0,corhft)
DIASht1<-DIAShft0[!J(1)][,.(menFantin=1-sum(Fantin)),keyby=.(IDRebano)]
DIASht2<-DIAShft0[J(1)][,.(prcom2=sum(prcom1)),keyby=.(IDRebano)]
DIASht3<-DIASht2[DIASht1[DIASht0]][,.(IDRebano,prcom3=ifelse(prcom2==0,0,
                      ifelse(is.na(menFantin),1/prcom2,menFantin/prcom2)))]

setkey(DIASht3,IDRebano);setkey(DIAShft0,IDRebano)
DIAShft1<-DIASht3[DIAShft0][,.(IDRebano,com2,prcom4=ifelse(corhft<1,corhft,prcom3))]

setkey(DIAShft1,IDRebano,com2);setkey(VPhift,IDRebano,com2)
DIAShift1<-DIAShft1[VPhift][,diashift1:=diashift0*prcom4][,
                  d1:=sum(diashift1),by=.(IDRebano)][,
                  diashift2:=ifelse(d1<.99999,0,diashift1)][]

# DIAShift1[,sum(diashift2),by=IDRebano] #Comprobar que sum por rebaño=1 ó 0

##########################

### Cálculo de la ingestión máxima de cada rebaño en cada ts: IMaxht
setkey(pl1$A1,t)
A1t<-pl1$A1[.(tt)][,.(IDRebano,Especie,Raza,Categoria,t,diay,n)]
...
setkey(pl1$A1b,t)
A1bt<-pl1$A1b[.(tt)]
setkey(A1t,IDRebano,Especie,Raza,Categoria,t,diay)
setkey(A1bt,IDRebano,Especie,Raza,Categoria,t,diay)
A1t2<-A1bt[A1t]

if(unique(A1t2$diay)==j){ #si un grupo hc de animales está en diay==j, su cc se pone a CCref
A1t20<-A1t2} else{
setkey(A1t2,IDRebano,Especie,Raza,Categoria)
setkey(Balance,IDRebano,Especie,Raza,Categoria)
A1t20<-Balance[A1t2][,.(IDRebano,Especie,Raza,Categoria,t=i.t,diay,
...
setkey(pl1$A0b,t)
A0bt<-pl1$A0b[.(tt)]
setkey(A1t20,IDRebano,Raza,Categoria,t,diay)
setkey(A0bt,IDRebano,Raza,Categoria,t,diay)
A1t2<-A0bt[A1t20][,.(IDRebano,Especie,Raza,Categoria,t,diay,n,cc,
...
setkey(IMaxht,IDRebano)
PPhift<-IMaxht[DIAShift1][,
  .(IDRebano,Especie,t,UP,IDMancha,com,com2,VP,diashift2,
...
### Cálculo de CIRhift
setkey(DIAShift1,IDRebano,IDMancha,com,com2)
setkey(PPhift,IDRebano,IDMancha,com,com2)
CIRhift01<-DIAShift1[PPhift][,.(IDRebano,Especie,t,UP,IDMancha,com,com2,sup,
        biovta,biomta,VP,diashift2,pphift,cirnum=diashift2*pphift,NhecesVhif,
        NhecesMhif,NorinVhif,NorinMhif,Fantin)][,
          cirden:=sum(cirnum),by=IDRebano][,
...
CIRhift02<-pl1$A4[CIRhift01][,msu:=biovta+biomta][,
    ch:=Tri(msu,linf,lsup,0,1)][,cirhift02:=ifelse(cirhift01<=ch,cirhift01,ch)][,
        mxcir2:=ifelse(ch<1,0,ifelse(cirhift02==Fantin,0,1))][,
          list(IDRebano,t,UP,IDMancha,com,com2,ch,Fantin,cirhift02,mxcir2,
...
#cond1<-sum(CIRhift02[,list(uno=sum(cirhift05)),by=list(IDRebano)][,round(uno,1)<1])
#cond2<-sum(CIRhift02[,cirhift02>Fantin])

# De momento no lo hacemos con "while". Loop "for" que se repita 4 veces.
for (m in 1:4){
  CIRht03<-CIRhift02[,.(tod=sum(cirhift02)),keyby=.(IDRebano,t,mxcir2)]
  setkey(CIRht03,mxcir2)
  CIRht030<-CIRht03[J(0)][,.(IDRebano,t,tod0=tod)]
  CIRht031<-CIRht03[J(1)][,.(IDRebano,t,tod1=tod)]
  CIRht032<-CIRht03[,length(mxcir2),.(IDRebano,t)]
  setkey(CIRht032,IDRebano,t);setkey(CIRht030,IDRebano,t)
  CIRht040<-CIRht030[CIRht032]
  setkey(CIRht040,IDRebano,t);setkey(CIRht031,IDRebano,t)
  CIRht04<-CIRht031[CIRht040][,tod1:=ifelse(is.na(tod1),0,tod1)][,
          tod0:=ifelse(is.na(tod0),0,tod0)]
  setkey(CIRht04,IDRebano,t);setkey(CIRhift02,IDRebano,t)
      ...
      cirhift02*(1-tod0)/tod1))][,
        cirhift02:=ifelse(cirhift02>Fantin,Fantin,cirhift02)][,
          mxcir2:=ifelse(cirhift02==Fantin,0,mxcir2)][,
            .(IDRebano,t,UP,IDMancha,com,com2,ch,Fantin,cirhift02,mxcir2,
            biovta,biomta,NhecesVhif,NhecesMhif,NorinVhif,NorinMhif)]
}
# Comprobar que suman 1
# CIRhift02[,sum(cirhift02),by=.(IDRebano)]
...
setkey(A1t2,Especie,Raza,Categoria)
IMaxhct<-A1t2[pl1$A6,nomatch=0][,.(IDRebano,Especie,Raza,Categoria,n,PV,cc,ingmax,
  iMaxhct=n*ingmax*ts*CorccI(cc))]

setkey(IMaxhct,IDRebano);setkey(CIRhift02,IDRebano)
IMaxhcift0<-IMaxhct[CIRhift02,allow.cartesian=T][,.(IDRebano,Especie,Raza,
...
IMaxhcift<-pl1$A4[IMaxhcift0][,.(Especie,IDRebano,Raza,Categoria,n,UP,IDMancha,
...
PRHhift<-pl1$D2B2B3[IMaxhcift][,list(hnum=sum(ihcift*probH)),
          by=list(IDRebano,t,IDMancha,com,com2)][,
                hden:=sum(hnum),by=list(IDRebano,t)][,
                    prH:=ifelse(hden==0,0,hnum/hden)][]

# PRHhift[,round(sum(prH),2),by=list(IDRebano,com2)][IDRebano==18]
########################

setkey(IMaxhcift,IDRebano,IDMancha,com,com2,t)
setkey(PRHhift,IDRebano,IDMancha,com,com2,t)
Ihcift00<-IMaxhcift[PRHhift][,list(IDRebano,Especie,Raza,Categoria,n,UP,IDMancha,
...
Ihct000<-Ihcift00[,list(NHhct=sum(NHvhcift+NHmhcift),
                  ...
                  keyby=list(IDRebano,Especie,Raza,Categoria,t)]
## Se multiplica el total de lo excretado por prH
setkey(Ihcift00,IDRebano,Especie,Raza,Categoria,t)
Ihcift0000<-Ihcift00[Ihct000][,list(IDRebano,Especie,Raza,Categoria,n,UP,
...
Ihcift<-Iift[Ihcift01][,list(IDRebano,Especie,Raza,Categoria,n,UP,IDMancha,com,
...
Nec<-Nec0[pl1$A4,nomatch=0][,
      .(IDRebano,Especie,Raza,Categoria,t,n,fcr,fcs,
  ...
  ENIgan:=(ivhcift*EM(DigV)*((KG1*DigV)+KG2))+(imhcift*EM(DigM)*((KG1*DigM)+KG2))][,
      .(EMIhc=sum(EMI),ENIhmd=sum(ENImd),
        ENIhgest=sum(ENIgest),ENIhl=sum(ENIl),ENIhgan=sum(ENIgan)),
         by=.(IDRebano,Especie,Raza,Categoria)][,
...
setkey(Nec,IDRebano,Especie,Raza,Categoria)
Balance<-Nec[ENIhc][,.(IDRebano,Especie,Raza,Categoria,t,n,PV,PVr,KganProm,
